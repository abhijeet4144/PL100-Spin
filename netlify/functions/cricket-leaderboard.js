...
exports.handler = async (event) => {
  ...
  try {
    ...
    const token  = process.env.WTC_BLOBS_TOKEN;

    if (!siteID) {
      return { statusCode: 500, headers: HEADERS, body: JSON.stringify({ error: 'Missing NETLIFY_SITE_ID env var' }) };
    }
    if (!token) {
      return { statusCode: 500, headers: HEADERS, body: JSON.stringify({ error: 'Missing WTC_BLOBS_TOKEN env var' }) };
    }

    store = getStore({ name: 'cricket-leaderboard', siteID, token });
  } catch (e) {
  ...
  if (event.httpMethod === 'POST') {
    ...
    if (typeof body.points !== 'number' || body.points < 0 || body.points > 36) {
